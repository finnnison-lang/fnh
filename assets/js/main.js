...
(() => {
  ...
  if (N && hasGsap && !reduce) {
    ...
    const tl = gsap.timeline({ scrollTrigger: { trigger: '.journey', start: 'top top', end: () => '+=' + ((N - 1) + HOLD + 0.4) * innerHeight, pin: '.journey__pin', scrub: 0.6, anticipatePin: 1, invalidateOnRefresh: true, onUpdate: () => { const t = tl.time(); const a = Math.min(N - 1, Math.max(0, Math.floor(t + (1 - HOLD)))); if (a !== tl._active) { tl._active = a; setActive(a); } }, onEnter: () => setActive(0), onEnterBack: () => setActive(tl._active || 0) } });
    for (let i = 1; i < N; i++) {
